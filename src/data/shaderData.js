const professionVert = `precision lowp float;
...
void main() {
    vNoise = pnoise(position + (uTime * 0.2), vec3(10.0));

    gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
}`;
const professionFrag = `precision lowp float;
...
const blobVert = `precision lowp float;
...
void main() {
    vNoise = pnoise((position + uTime * 0.5) * 0.2, vec3(10.0));
    float displacement = (vNoise / 1.0);
    vec3 newPosition = position + normal * displacement;
    vec4 positionViewSpace = modelViewMatrix * vec4(newPosition, 1.0);
    vPositionViewSpace = positionViewSpace.xyz;
    gl_Position = projectionMatrix * positionViewSpace;

    gl_PointSize = 4.0;
}`;
const blobFrag = `precision lowp float;
...
export { professionVert, professionFrag, blobVert, blobFrag };
